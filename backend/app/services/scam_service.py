...
def analyze_scam(text: str):
    ...
    matched = [word for word in SCAM_KEYWORDS if word in lowered]

    score = min(sum(SCAM_KEYWORDS[word] for word in matched), 100)
    ...
    warnings = [f"Suspicious phrase detected: {word}" for word in matched]
